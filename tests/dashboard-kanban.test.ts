...
describe("dashboard kanban view", () => {
  it("serves a lane-grouped snapshot at /api/runs/:id/kanban", () => {
    ...
    try {
      const result = runNodeScript(
        `
          ...
          try {
            ...
            // The legacy HTML kanban endpoint now redirects to the React SPA
            const redirectRes = await fetch(baseUrl + "/runs/" + runId + "/kanban", { redirect: "manual" });
            ...
            console.log(JSON.stringify({
              ...
              redirectStatus: redirectRes.status,
              redirectLocation: redirectRes.headers.get("location"),
      ...
      assert.equal(result.redirectStatus, 302, "legacy kanban endpoint should redirect to React SPA");
      assert.equal(result.redirectLocation, "/kanban");

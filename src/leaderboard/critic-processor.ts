...
// MIGRATED TO PRISMA — all repo methods are now async
// ══════════════════════════════════════════════════════════════════════
 ...
 *
 * NOTE: now async because Prisma-based repository is async.
 */
export async function processCriticOutput(
  output: string,
  repository: LeaderboardRepository,
): Promise<{ rejected: number; audited: number }> {
  ...
  for (const r of rejections) {
    try {
      await repository.reject(r.experimentId, r.reason);
    } catch {

...
describe("getSystemTokenSpend", () => {
  ...
  before(async () => {
    ...
    startingSpend = await getSystemTokenSpend();
  });
  ...
  it("returns updated value after incrementSystemTokenSpend", async () => {
    await incrementSystemTokenSpend(100);
    const result = await getSystemTokenSpend();
    assert.equal(result, 100);
  });

  it("accumulates across multiple increments", async () => {
    await incrementSystemTokenSpend(50);
    await incrementSystemTokenSpend(25);
    const result = await getSystemTokenSpend();

...
Salve um relatório narrativo de cada rodada no banco via `save_artifact`. Este é o equivalente do relatório de modelagem do agentic-ml, mas no banco (fonte da verdade) — o reporter e os outros times o consomem via API. **NÃO basta gerar arquivo `.md` legado; o banco é a fonte da verdade.** Inclua sempre a métrica de **decorrelação** (Spearman vs top-1) — é sua contribuição principal para o ensemble.

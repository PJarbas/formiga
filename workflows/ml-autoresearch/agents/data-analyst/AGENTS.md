...
Use a tool `save_artifact` com `key = "eda_report"`:
...
Sempre que decidir remover uma coluna, aplicar transformação forte, ou identificar leakage, registre com `log_decision`:

```
log_decision({
  "decision_type": "feature_drop",
  "description": "Recomendando remover 'order_status' — metadado pós-evento",
  "reasoning": "Correlação de 0.98 com target indica leakage temporal",
  "alternatives_considered": ["target-encode com validação por fold", "manter e monitorar"]
})
```

### Passo 4: Reportar Métricas Numéricas Chave
